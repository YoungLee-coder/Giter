...
- Prefer Edit over Write for existing files. Frontend IPC goes through `api` in `src/lib/tauri.ts`; git/gh work stays in `src-tauri/src/git/`. Verify with `pnpm typecheck` / `pnpm test:unit` or `cargo test --manifest-path src-tauri/Cargo.toml` per `.ai/workflow.md`.
