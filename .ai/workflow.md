...
- Notes：`.github/scripts/build-release-notes.sh` 抽 CHANGELOG 对应段，并附各平台安装包直链（macOS Apple Silicon / Intel `.dmg`，Windows NSIS / MSI）。本地预览：`bash .github/scripts/build-release-notes.sh {version}`。
