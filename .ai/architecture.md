...
- `src/App.tsx` — 主界面：仓库网格、批量栏、工作区整页、设置页切换、更新提示。
- `src/components/` — UI。`repo/` 为仓库卡片/网格/批量栏；`workspace/` 为单仓库工作区；`ui/` 为 shadcn（radix-nova）；`SettingsPage.tsx` 为设置整页。
- `src/hooks/` — `useRepos` / `useSettings` / `useI18n` / `useRepoDragSort` / `useWorkspace`；`AppUiProvider` 管选中态、设置开关、工作区仓库、批量进度。
...
- `src-tauri/src/commands.rs` — IPC 命令层：并发、`spawn_blocking`、批量任务互斥、按仓库锁（`RepoLocks`，同仓库命令排队而非报错）。
- 工作区打开时 `workspace_snapshot` 与 `repo_refs` 都经同一把按仓库锁串行执行；前端在 snapshot 就绪后再预取 refs（不按标签页懒加载），切到分支页时直接命中缓存。
- `src-tauri/src/git/` — 系统 `git` / `gh` 调用、状态解析、远程 provider、FF-only 更新、工作区写入、GitHub 发布。
...
- `src-tauri/src/git/` 拥有全部 git/gh CLI、状态解析、provider 检测、`update_one` 跳过规则、工作区写入。改动后跑 `cargo test --manifest-path src-tauri/Cargo.toml`。
- `src-tauri/src/store.rs` 拥有仓库根判定（含 `.git` 文件 / 空 stub 拒绝）。改动后跑同一 `cargo test`。
- 舱队批量（`batch_fetch` / `batch_update` / `update_one`）不得调用 stash / merge / rebase。工作区命令与批量路径隔离。
